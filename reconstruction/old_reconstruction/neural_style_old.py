...
class NeuralStyleTransfer:
    ...
    def __init__(self, content_image_path, style_image_path, 
        ...
        self.content_extractor = VggFeatureExtractor(
            target_layer=content_layer, device=device, model_type=model_type
        )
        
        # Initialize style feature extractors
        print(f"Initializing style feature extractors (VGG {model_type}, layers {style_layers})...")
        ...
        with torch.no_grad():
            self.content_features = self.content_extractor(self.content_image)
    ...
    def transfer(self, optimizer_type="lbfgs", init_method="content", 
        ...
        def closure():
            ...
            current_content_features = self.content_extractor(generated)
            content_loss = nn.functional.mse_loss(current_content_features, self.content_features)
            ...
            for layer, extractor in self.style_extractors.items():
                ...
                layer_style_loss = nn.functional.mse_loss(current_gram, self.style_features[layer])
                total_style_loss += layer_style_loss / len(self.style_extractors)
